    ...
    if ($email === '' || $password === '' || !isset($mysqli) || !($mysqli instanceof mysqli)) {
        ...
        if (!$user) {
            ...
            if (!$ok) {
                ...
                // Nếu có phân quyền admin sau này có thể check account_type ở đây
                // $role = (int)($user['account_type'] ?? 0);
                // if ($role == 1) { header('Location: admin/index.php'); exit; }
    ...
    <div class="form-box">
        <div class="form-value">
            <?php if (!empty($login_error)): ?>
                <div class="alert alert-error" style="margin-bottom:16px;padding:10px;border:1px solid #f00;color:#c00;">
...
<?php
// Chỉ giữ lại toast cho trường hợp ?message=success (đăng nhập thành công)
