...
if (session_status() === PHP_SESSION_NONE) {
    // Phải trùng với header.php & login.php
    session_name('guha');
    session_start();
}
...
if (isset($_GET['update'])) {
    ...
    foreach ($_SESSION['cart'] as $cart_item) {
        ...
        if ($qty_input === 0) {
            // không push cart_item -> bị xoá
            continue;
        }

        // TC5: số lượng vượt quá tồn kho
...
if (isset($_POST['addtocart']) || isset($_POST['buynow'])) {
    ...
    if ($row) {
        ...
        $new_product = [[
            ...
            'product_image'    => $img_file, // lưu TÊN FILE
            'capacity_name'    => $row['capacity_name'] ?? '',
        ]];

        /* ====== FLOW MUA NGAY: KHÔNG ĐỤNG VÀO GIỎ HÀNG ====== */
        if (isset($_POST['buynow'])) {
            // Lưu riêng sản phẩm mua ngay để checkout dùng
            $_SESSION['buynow'] = $new_product[0];

            header('Location: ../../index.php?page=checkout&mode=buynow');
            exit;
        }
        /* =================================================== */

        // ==== FLOW THÊM GIỎ HÀNG BÌNH THƯỜNG ====
        $found   = false;
        $product = [];
    ...
    // thêm giỏ hàng xong thì quay lại trang trước với message
